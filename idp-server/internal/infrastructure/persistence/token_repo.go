...
type TokenRepository struct {
	db                       dbRouter
	forcePrimaryReadBySHA256 bool
}

func NewTokenRepository(db *sql.DB) *TokenRepository {
	return NewTokenRepositoryRW(db, nil)
}

func NewTokenRepositoryRW(writeDB, readDB *sql.DB) *TokenRepository {
	return NewTokenRepositoryRWWithPolicy(writeDB, readDB, false)
}

func NewTokenRepositoryRWWithPolicy(writeDB, readDB *sql.DB, forcePrimaryReadBySHA256 bool) *TokenRepository {
	return &TokenRepository{
		db:                       newDBRouter(writeDB, readDB),
		forcePrimaryReadBySHA256: forcePrimaryReadBySHA256,
	}
}
...
func (r *TokenRepository) FindActiveAccessTokenBySHA256(ctx context.Context, tokenSHA256 string) (*tokendomain.AccessToken, error) {
	...
	db := r.db.reader()
	if r.forcePrimaryReadBySHA256 {
		db = r.db.writer()
	}
	row := db.QueryRowContext(ctx, tokenRepositorySQL.findActiveAccessBySHA256, tokenSHA256)
...
func (r *TokenRepository) FindActiveRefreshTokenBySHA256(ctx context.Context, tokenSHA256 string) (*tokendomain.RefreshToken, error) {
	...
	row := db.QueryRowContext(ctx, tokenRepositorySQL.findActiveRefreshBySHA256, tokenSHA256)
...
func (r *TokenRepository) ListActiveAccessTokensByUserID(ctx context.Context, userID int64) ([]*tokendomain.AccessToken, error) {
	...
	rows, err := r.db.reader().QueryContext(ctx, tokenRepositorySQL.listActiveAccessByUserID, userID)
	if err != nil {
		return nil, err
	}
...
func (r *TokenRepository) ListActiveRefreshTokensByUserID(ctx context.Context, userID int64) ([]*tokendomain.RefreshToken, error) {
	...
	rows, err := r.db.reader().QueryContext(ctx, tokenRepositorySQL.listActiveRefreshByUserID, userID)
	if err != nil {
		return nil, err
	}
	defer func() { _ = rows.Close() }()
...
func (r *TokenRepository) RevokeAccessTokensByUserID(ctx context.Context, userID int64, revokedAt time.Time) error {
	...
	_, err := r.db.writer().ExecContext(ctx, tokenRepositorySQL.revokeAccessByUserID, revokedAt, userID)
	return err
}
...
func (r *TokenRepository) RevokeRefreshTokensByUserID(ctx context.Context, userID int64, revokedAt time.Time) error {
	...
	_, err := r.db.writer().ExecContext(ctx, tokenRepositorySQL.revokeRefreshByUserID, revokedAt, userID)
	return err
}
