...
services:
  db:
    ...
    environment:
      MYSQL_ROOT_PASSWORD: ${IDP_MYSQL_ROOT_PASSWORD:-rootpass}
      MYSQL_DATABASE: ${IDP_MYSQL_DATABASE:-app}
      MYSQL_USER: ${IDP_MYSQL_USER:-app}
      MYSQL_PASSWORD: ${IDP_MYSQL_PASSWORD:-apppass}
    ports:
      - "${IDP_MYSQL_PORT:-3306}:3306"
    volumes:
      - idp-mysql-data:/var/lib/mysql
      - ./idp-server/scripts/migrate.sql:/docker-entrypoint-initdb.d/001-migrate.sql:ro
    healthcheck:
  ...
  server:
    image: ${IDP_SERVER_IMAGE:-ghcr.io/wsplxa/oauth2-sienne-idp-server:latest}
    restart: unless-stopped
    environment:
      MYSQL_HOST: db
      MYSQL_PORT: 3306
      ...
      SIGNING_KEY_ROTATE_BEFORE: ${IDP_SIGNING_KEY_ROTATE_BEFORE:-24h}
      SIGNING_KEY_RETIRE_AFTER: ${IDP_SIGNING_KEY_RETIRE_AFTER:-24h}
    ports:
      - "${IDP_SERVER_PORT:-8080}:8080"
    depends_on:
      db:
      ...
      redis:
        condition: service_healthy
    healthcheck:
      test: ["CMD", "sh", "-c", "exec 3<>/dev/tcp/127.0.0.1/8080 && printf 'GET /healthz HTTP/1.1\r\nHost: localhost\r\nConnection: close\r\n\r\n' >&3 && grep -q '200 OK' <&3"]
      interval: 15s
      timeout: 5s
      retries: 20
      start_period: 20s
...
$composeFile = Join-Path (Get-Location) 'compose.quickstart.yaml'
$migrateDir = Join-Path (Get-Location) 'idp-server/scripts'
$migrateFile = Join-Path $migrateDir 'migrate.sql'
...
try {
  ...
  Write-Error 'docker compose not available. Make sure Docker Compose v2 is installed.'
}

if (-not (Test-Path $composeFile)) {
  Write-Host 'Writing compose file: compose.quickstart.yaml'
  Set-Content -Path $composeFile -Value $composeContent -Encoding UTF8
}

if (-not (Test-Path $migrateFile)) {
  Write-Host 'Writing migrate.sql to idp-server/scripts/migrate.sql'
  New-Item -ItemType Directory -Path $migrateDir -Force | Out-Null
  Set-Content -Path $migrateFile -Value $migrateContent -Encoding UTF8
}
